...
public class CuttingBoardRecipeBuilder implements RecipeBuilder
{
	...
	public CuttingBoardRecipeBuilder(Ingredient ingredient, Ingredient tool, ItemLike mainResult, int count, float chance) {
		this.results.add(new ChanceResult(ItemStackTemplate.fromNonEmptyStack(new ItemStack(mainResult.asItem(), count)), chance));
		this.ingredient = ingredient;
		this.tool = tool;
		this.resultItem = mainResult.asItem();
	...
	public CuttingBoardRecipeBuilder addResult(ItemLike result, int count) {
		this.results.add(new ChanceResult(ItemStackTemplate.fromNonEmptyStack(new ItemStack(result.asItem(), count)), 1));
		return this;
	}
	...
	public CuttingBoardRecipeBuilder addResultWithChance(ItemLike result, float chance, int count) {
		this.results.add(new ChanceResult(ItemStackTemplate.fromNonEmptyStack(new ItemStack(result.asItem(), count)), chance));
		return this;
	}
	...
	public void build(RecipeOutput outputIn, String save) {
		...
		if ((Identifier.parse(save)).equals(resourcelocation)) {
			...
			this.save(outputIn, ResourceKey.create(Registries.RECIPE, Identifier.parse(save)));
		}
	}

	public void build(RecipeOutput output, Identifier id) {
		save(output, net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.RECIPE, id));
	}

	@Override
	public ResourceKey<net.minecraft.world.item.crafting.Recipe<?>> defaultId() {
		Identifier id = Identifier.fromNamespaceAndPath(this.namespace != null ? this.namespace : getDefaultRecipeId(this.resultItem).getNamespace(), getDefaultRecipeId(this.resultItem).getPath()).withPrefix(folder.getSerializedName() + "/");
