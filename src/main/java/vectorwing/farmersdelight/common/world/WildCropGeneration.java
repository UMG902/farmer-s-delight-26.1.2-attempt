...
public class WildCropGeneration
{
        ...
        public static void bootstrapConfiguredFeatures(BootstrapContext<ConfiguredFeature<?, ?>> context) {
                ...
                context.register(FEATURE_PATCH_WILD_TOMATOES, wildCropConfiguredFeature(
                        ModBlocks.WILD_TOMATOES.get(), Blocks.DEAD_BUSH, BlockTags.SUBSTRATE_OVERWORLD
                ));
        ...
        public static void bootstrapPlacedFeatures(BootstrapContext<PlacedFeature> context) {
                ...
                context.register(PATCH_WILD_RICE, createWildRicePlacedFeature(configuredFeatureLookup, FEATURE_PATCH_WILD_RICE, 20));
        ...
        private static ConfiguredFeature<?, ?> defaultWildCropConfiguredFeature(Holder<PlacedFeature> primaryFeature, Holder<PlacedFeature> secondaryFeature, @Nullable Holder<PlacedFeature> floorFeature) {
                // tries=1 + count=64 in placed feature: matches FDR's per-call single placement spread across the chunk
                // (was tries=64 with no count -> one tiny cluster per qualifying chunk, making patches essentially invisible)
                return new ConfiguredFeature<>(
                        ModBiomeFeatures.WILD_CROP.get(),
                        new WildCropConfiguration(1, 4, 3, primaryFeature, secondaryFeature, floorFeature)
                );
        }
        ...
        private static PlacedFeature createWildRicePlacedFeature(HolderGetter<ConfiguredFeature<?, ?>> featureGetter, ResourceKey<ConfiguredFeature<?, ?>> feature, int rarity) {
                return new PlacedFeature(featureGetter.getOrThrow(feature), List.of(
                        RarityFilter.onAverageOnceEvery(rarity),
                        InSquarePlacement.spread(),
